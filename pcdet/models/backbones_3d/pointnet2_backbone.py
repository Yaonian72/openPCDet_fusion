...
from ..backbones_3d.pfe import vote_module
...
class PointNet2MSG_FPS(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINTS.__len__()):
            ...
            self.SA_modules.append(
                pointnet2_modules.PointnetSAModuleMSG_FPS(
                    npoint=self.model_cfg.SA_CONFIG.NPOINTS[k],
                    radii=self.model_cfg.SA_CONFIG.RADIUS[k],
                    nsamples=self.model_cfg.SA_CONFIG.NSAMPLE[k],
                    mlps=mlps,
                    dilated_group=self.model_cfg.SA_CONFIG.DILATED_GROUP[k],
                    use_xyz=self.model_cfg.SA_CONFIG.get('USE_XYZ', True),
                    fps_mod=self.model_cfg.SA_CONFIG.FPS_MOD[k],
                    fps_sample_range_list=self.model_cfg.SA_CONFIG.FPS_SAMPLE_RANGE[k]
                )
            )
            # skip_channel_list.append(channel_out)
            channel_in = channel_out
            ...
            self.aggregation_mlps.append(
                nn.Sequential(
                    nn.Conv1d(channel_in, self.model_cfg.AGGREGATION_CHANNELS[k], kernel_size=1, bias=False),
                    nn.BatchNorm1d(self.model_cfg.AGGREGATION_CHANNELS[k]),
                    nn.ReLU()))

            channel_out = self.model_cfg.AGGREGATION_CHANNELS[k]
            skip_channel_list.append(channel_out)
            channel_in = self.model_cfg.AGGREGATION_CHANNELS[k]
        ...
        if self.model_cfg.FP_MLPS.__len__() > 0:
            self.num_point_features = self.model_cfg.FP_MLPS[0][-1]
        else:
    ...
    def forward(self, batch_dict):
        ...
        features = features.view(batch_size, -1, features.shape[-1]).permute(0, 2, 1).contiguous() if features is not None else None # (B, C, N)

        # SA modules feed forward
        l_xyz, l_features = [xyz], [features]
        for i in range(len(self.SA_modules)):
            li_xyz, li_features = self.SA_modules[i](l_xyz[i], l_features[i])
            li_features = self.aggregation_mlps[i](li_features)
            l_xyz.append(li_xyz)
            l_features.append(li_features)
        ...
        if len(self.FP_modules) == 0:
            ...
            point_features = l_features[0].permute(0, 2, 1).contiguous()  # (B, N, C)
            batch_dict['point_features'] = point_features.view(-1, point_features.shape[-1])
            batch_idx = torch.arange(batch_size, device=l_xyz[0].device).view(-1, 1).repeat(1, l_xyz[0].shape[1]).view(-1)
            batch_dict['point_coords'] = torch.cat((batch_idx[:, None].float(), l_xyz[0].view(-1, 3)), dim=1)
        return batch_dict
